...
if [ $STAGE == $STAGE_PYTHON ]; then
  ...
  while [[ $EXIT_CODE == 0 ]]; do
	  run_with_watchdog "run_mvn $MVN_COMMON_OPTIONS $MVN_TEST_OPTIONS $PROFILE -pl flink-runtime verify -Dtest=ZooKeeperMultipleComponentLeaderElectionDriverTest#testLeaderElectionWithMultipleDrivers" $CALLBACK_ON_TIMEOUT
	  EXIT_CODE=$?
	done
fi
