...
public class CheckpointResourcesCleanupRunner implements JobManagerRunner {
        ...
        try {
            this.completedCheckpointStore =
                    checkpointRecoveryFactory.createRecoveredCompletedCheckpointStore(
                            getJobID(), jobManagerConfiguration, LOG);

            loadSharedStateFromCheckpoints(completedCheckpointStore);
        } catch (Exception e) {
    ...
    private void loadSharedStateFromCheckpoints(CompletedCheckpointStore completedCheckpointStore)
            throws Exception {
        try (SharedStateRegistry sharedStateRegistry =
                sharedStateRegistryFactory.create(cleanupExecutor)) {
    ...
    public CompletableFuture<Acknowledge> cancel(Time timeout) {
        Preconditions.checkState(
                resultFuture != null, "The CheckpointResourcesCleanupRunner was not started, yet.");
